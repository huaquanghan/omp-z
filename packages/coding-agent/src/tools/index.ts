...
export async function resolveBuiltinToolPlan(session: ToolSession, toolNames?: string[]): Promise<BuiltinToolPlan> {
	...
	if (requestedTools && !restrictToolNames) {
		...
		if (["hindsight", "mnemopi", "zvec"].includes(cfgMemoryBackend.get(session.settings))) {
			for (const name of ["recall", "retain", "reflect"]) {
				if (!requestedTools.includes(name)) requestedTools.push(name);
			}
		}
		if (
			["mnemopi", "zvec"].includes(cfgMemoryBackend.get(session.settings)) &&
		...
		if (cfgAutolearnEnabled.get(session.settings) && (session.taskDepth ?? 0) === 0) {
			...
			if (
				["hindsight", "mnemopi", "local", "zvec"].includes(cfgMemoryBackend.get(session.settings)) &&
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "retain" || name === "recall" || name === "reflect") {
			return ["hindsight", "mnemopi", "zvec"].includes(cfgMemoryBackend.get(session.settings));
		}
		if (name === "memory_edit") return ["mnemopi", "zvec"].includes(cfgMemoryBackend.get(session.settings));
